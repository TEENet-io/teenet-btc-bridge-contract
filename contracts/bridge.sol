...
contract TEENetBtcEvmBridge is ITEENetBtcEvmBridgeErrors {
    ...
    /// @param rx (rx, s) forms a Schnorr signature on keccak256(btcTxId || receiver || amount). Function abi.encode is used to perform concatenation.
    /// @param s .
    function mint(
        bytes32 btcTxId,
        address receiver,
    ...
    /// The function is performed by users who want to redeem BTC.
    ///
    ...
    /// Prepare to redeem BTC. The function is performed by bridge nodes 
    /// via m-out-of-n threshold Schnorr signature scheme.
    /// After the bridge nodes observe the redeem request, they look for 
    /// the spendable BTC coins that can be used to construct the BTC tx. 
    /// They then record the spendables within the bridge contract to
    /// avoid sending multiple BTC txs for the same redeem request.
    ///
    /// @param redeemRequestTxHash Hash of the tx that sent by the user to request redeem btc
    ...
    /// @param rx (rx, s) forms a Schnorr signature on keccak256(redeemRequestTxHash||requester||amount||[btcTxId || idx]). Function abi.encode is used to perform concatenation.
    /// @param s .
